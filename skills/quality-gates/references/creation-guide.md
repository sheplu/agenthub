...
How to bootstrap the gate workflows for a repo that has none. Read
[conventions.md](conventions.md) first — the workflows you produce must satisfy
all of it.
...
2. Write the three workflow files — `quality-gates.yaml`, `sast.yaml`,
   `dependency-scan.yaml` — with the canonical shape and each job in its
   designated file.
